...
class RunningLog:
    ...
    @classmethod
    def add_log(cls, content, level=1, logger="undefined", is_print=False, add_period=True, **sth):
        ...
        :return:
        """
        log = "[%s] %s %s: %s" % (log_level_list[level], get_formatted_time(), logger, content)
    ...
    def write_log(cls):
        ...
        for i in range(0, cls.log_queue.qsize()):
            try:
